...
export class ChartService {
        ...
        getMonthChartData(resp, type, todayDate){
              ...
              return new Promise((resolve,reject)=>{
                  this.arrangeData(resp, type).then(result=>{
                    for(var r_idx=1; r_idx<=this.getDateInMonth(result[0].date); r_idx++){
                      for(var c_idx=1; c_idx<=Object.keys(result).length; c_idx++){
                        if(result[c_idx-1].date.getDate() == r_idx){
                          if(type == "Bloodpressure"){
                            data_l[r_idx-1] = result[c_idx-1].value_l;
                            data_h[r_idx-1] = result[c_idx-1].value_h;
                          }else{
                            data_l[r_idx-1] = result[c_idx-1].value;
                          }
                        }// can not use else here I don't know why but 0 will replace the real value

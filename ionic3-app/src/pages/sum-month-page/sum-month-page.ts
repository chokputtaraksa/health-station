...
@IonicPage()
@Component({
  selector: 'sum-month-page',
  templateUrl: 'sum-month-page.html',
  providers: [DataService,]
})
export class SumMonthPage {
    ...
    whichTime : Date;
    info : object; // information before create chart
    ...
    ionViewDidLoad() {
      ...
      this.dataService.getDataByPeriod(this.uid, "month", this.title).then((resp)=>{
    ...
    prepareDataForChart(resp, type, whichTime){
      ...
      return new Promise((resolve,reject)=>{
          this.arrangeData(resp, type).then(result=>{
            ...
            for(var i=1; i<=this.getDateInMonth(new Date(whichTime.getFullYear(), whichTime.getMonth())); i++){
              if(!data_l[i-1]){
                data_l[i-1]=0;
              }
              if(!data_h[i-1]){
